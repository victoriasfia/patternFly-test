...
import {Avatar, Brand, Breadcrumb, ToolbarToggleGroup, BreadcrumbItem, Button, ButtonVariant, Card, CardBody, Content, Divider, Dropdown, 
...
export default function App() {
  ...
  const headerToolbar = <Toolbar id="toolbar" isStatic>
      <ToolbarContent>
    ...
    md: 'gapMd'
          ...
          <ToolbarGroup variant="action-group-plain" visibility={{
    default: 'hidden',
    lg: 'visible'
  }}>
            <ToolbarItem>
    ...
    position: 'right'
  }} toggle={toggleRef => <MenuToggle ref={toggleRef} onClick={onKebabDropdownToggle} isExpanded={isKebabDropdownOpen} variant="plain" aria-label="Settings and help" icon={<EllipsisVIcon />} />}>
    ...
    position: 'right'
  }} toggle={toggleRef => <MenuToggle ref={toggleRef} onClick={onFullKebabDropdownToggle} isExpanded={isFullKebabDropdownOpen} variant="plain" aria-label="Toolbar menu" icon={<EllipsisVIcon />} />}>
  ...
  const masthead = <Masthead>
      <MastheadMain>
        ...
        <MastheadBrand>
          <MastheadLogo>
            <Brand src={pfLogo} alt="PatternFly" heights={{
    default: '36px'
  }} />
          </MastheadLogo>
        </MastheadBrand>
  ...
  const pageSkipToContent = <SkipToContent onClick={handleClick} href={`#${mainContainerId}`}>
      Skip to content
    </SkipToContent>;
  return <Page masthead={masthead} sidebar={sidebar} isManagedSidebar skipToContent={pageSkipToContent} mainContainerId={mainContainerId}>
      <PageGroup stickyOnBreakpoint={{
    default: 'top'
  }}>
      ...
      <PageSection aria-label="Card gallery">
        <Gallery hasGutter>
          {Array.from({
    length: 50
  }).map((_value, index) => <GalleryItem key={index}>
    
            <Card>
              <CardBody>
                <Content>
                  <h2>Card title {index + 1}</h2>
                  <p>This is some card content. This is some card content. This is some card content.</p>
                </Content>
              </CardBody>
            </Card>
            </GalleryItem>)}
